...
`.gitattributes` routes `*.dds *.png *.grle *.gdm *.shapes` through Git LFS.
`map/map.i3d` is **deliberately excluded** from LFS — kept as plain
diffable text specifically so node/UserAttribute regressions show up in
`git diff` — even though it's grown past GitHub's 50MB soft-warning
threshold (currently ~54MB; hard block is 100MB, watch this if it keeps
growing). `*.i3d.terrain.*.cache` files are GE-regenerated and gitignored.
